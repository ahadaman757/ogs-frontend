...
const getBoxesDetails = async (req, res, next) => {
  try {
    ...
    const shortlisted = await sequelize.query(
      `SELECT * FROM job_applicants_cv WHERE job_id = '${jobId}' AND 	is_shortlisted = '1'`
    );
    const rejected = await sequelize.query(
      `SELECT * FROM job_applicants_cv WHERE job_id = '${jobId}' AND 	is_rejected = '1'`
    );
    res.json({
      code: 1,
...
const getJobByIndustry = async (req, res, next) => {
  console.log("view all jobs");
  const { title } = req.body;
  try {
    const [company_jobs_record, meta] =
     ...
     left outer JOIN maxexperiences maxExperience on j.max_experience_id=maxExperience.id
     left outer JOIN maxexperiences minExperience on j.min_experience_id=minExperience.id
     WHERE j.functional_area_id REGEXP '${title}'
    `);
    console.log(meta);
    if (company_jobs_record.length > 0) {
      res.json({ code: 1, jobs: company_jobs_record });
