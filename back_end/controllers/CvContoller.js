...
const UpdateCv = async (req, res, next) => {
  ...
  try {
    ...
    if (req.files?.passport_photo == null) {
      ...
      fs.unlink("images/" + cv_image.passport_photo_pre("images", ""), (err) => {
        if (err) {
          console.log("error")
          console.log(err)
          next(err)
