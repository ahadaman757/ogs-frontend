...
const Accountsetting = () => {
  ...
  const resetFormik = useFormik({
    ...
    onSubmit: (values) => {
      axios
        .post('https://3.110.201.21:3002/users/resetpassword', values, {
          headers: {
    ...
    validationSchema: Yup.object({
      ...
      new_repeat_password: Yup.string().oneOf(
    ...
    <div className={`row p-4 ${Styles.Accountsettingmain}`}>
      <form onSubmit={resetFormik.handleSubmit}>
        ...
        <div className="row">
          ...
          {formresponse && <p className="text-success">{formresponse}</p>}
          <div className="col-md-6 d-flex justify-content-end my-3">
            <button type="submit" className={` ${Styles.profsbtn}`}>
              Save
            </button>
          </div>
        </div>
