...
const Jobcard = ({ job_data }) => {
  ...
  const CheckApplied = () => {
    ...
    axios.post("http://3.110.201.2:3002/jobs/checkjobapply", {
      job_id: job_id
    }, {
      headers: {
    ...
    <div className={`p-3 my-2 ${Styles.Jobcardmain}`}>
      <div className=" d-flex align-items-center">
        <div className="d-flex">
          ...
          {
            Applied ? <p className="ogsfonts14 m-1 text-bold text-success">Applied </p> : <button onClick={() => navigateToJobDetails(AppliedCvs)} className="btn-primary px-2 text-light unset_button">Apply for Job</button>
          }
